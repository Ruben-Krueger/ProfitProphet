...
export const generateMockData = () => {
  ...
  const types = ['Price Arbitrage', 'Time Arbitrage', 'Cross-Platform', 'Odds Mismatch'];
  ...
  const opportunities = Array.from({ length: 15 }, (_, index) => ({
    ...
    status: ['active', 'completed', 'expired'][Math.floor(Math.random() * 3)] as 'active' | 'completed' | 'expired',
    timeFound: new Date(Date.now() - Math.random() * 86400000).toLocaleTimeString() // Random time in last 24h
  }));
