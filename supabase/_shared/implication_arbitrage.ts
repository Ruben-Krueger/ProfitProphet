...
import { Market, ArbitrageOpportunity, Config, NumericalExtraction, ImplicationRule } from '../types';
...
export class ImplicationArbitrageCalculator {
  ...
  private extractNumericalMarkets(markets: Market[]): Array<Market & { numerical: NumericalExtraction }> {
    return markets
      .map(market => ({
        ...market,
        numerical: this.extractNumericalInfo(market.question)
      }))
      .filter(market => market.numerical.value !== null) as Array<Market & { numerical: NumericalExtraction }>;
  }
  ...
  private extractNumericalInfo(question: string): NumericalExtraction {
    ...
    const patterns = [
      ...
      { 
        regex: /unemployment.*?(?:above|over|exceed|higher than|greater than)\s*(\d+(?:\.\d+)?)\s*%/i,
        metric: 'unemployment_rate',
        direction: 'above' as const,
        unit: '%'
      },
      { 
        regex: /unemployment.*?(?:below|under|less than|lower than)\s*(\d+(?:\.\d+)?)\s*%/i,
        metric: 'unemployment_rate',
      ...
      // Bitcoin patterns
      { 
        regex: /Bitcoin.*?(?:above|over|exceed|reach|hit)\s*\$?(\d+(?:,\d+)?(?:k|K)?)/i,
      ...
      { 
        regex: /(?:above|over|exceed|more than|greater than)\s*(\d+(?:\.\d+)?)/i,
  ...
  private groupByMetric(markets: Array<Market & { numerical: NumericalExtraction }>): Map<string, Array<Market & { numerical: NumericalExtraction }>> {
  ...
  private findThresholdViolationsForMetric(markets: Array<Market & { numerical: NumericalExtraction }>): ArbitrageOpportunity[] {
    const opportunities: ArbitrageOpportunity[] = [];
    
    // Sort by threshold value
    const sortedMarkets = markets.sort((a, b) => a.numerical.value - b.numerical.value);
